const messages = {
    EMAIL_ALREADY_REGISTERED: "Email already registered",
    SIGNUP_FAILED: "Signup failed. Please try again.",
    LOGIN_SUCCESSFULL: "Login successful!",
    INTERNAL_SERVER_ERROR: "Internal server error.",
    SIGNUP_SUCCESSFULL: `Signup successful! Please check your email`,
    USER_NOT_FOUND: "User not found",
    DRIVER_NOT_FOUND: "Driver not found",
    PACKAGE_NOT_FOUND: "Package not found",
    ACCESS_DENIED: "Access Denied",
    INVALID_TOKEN: "Invalid Token",
    VERIFY_EMAIL_FIRST: "Please verify your email first.",
    INVALID_CREDENTIALS: "Invalid credentials",
    PROFILE_UPDATED: "Profile updated successfully!",
    NO_PROFILE_CHANGES: "No changes made to the profile.",
    PASSWORD_REQUIRED: "Both old and new passwords are required",
    INCORRECT_OLD_PASSWORD: "Incorrect old password",
    PASSWORD_CHANGED: "Password changed successfully",
    PASSWORD_CHANGE_FAILED: "Unable to change password",
    RECOVERY_EMAIL_SENT: "A recovery email has been sent to your email address to reset your password",
    PASSWORD_UPDATED: "Password updated successfully!",
    DRIVER_DOCUMENT_CHANGE_REQUEST_SUCCESSFULL: "Document change request submitted successfully!",
    DRIVER_DOCUMENT_CHANGE_REQUEST_FAILED: "Document change request failed!",
    USER_CREATED: "User created successfully.",
    DRIVER_CREATED: "Driver created successfully.",
    USER_DELETED: "User deleted successfully.",
    DRIVER_DELETED: "Driver deleted successfully.",
    PACKAGE_DELETED: "Package deleted successfully.",
    DATA_RETRIEVED: "Data retrieved successfully.",
    PACKAGE_SENT: "Package sent successfully.",
    PACKAGE_CREATED: "Package created successfully.",
    SOMETHING_WENT_WRONG: "Something went wrong!.",
    PICKUP_ADDRESS_NOT_FOUND: "Pickup address not found.",
    PICKUP_ADDRESS_CREATED: "Pickup address created successfully.",
    PICKUP_ADDRESS_UPDATED: "Pickup address updated successfully.",
    PICKUP_ADDRESS_DELETED: "Pickup address deleted successfully.",
    DROPOFF_ADDRESS_NOT_FOUND: "Dropoff address not found.",
    DROPOFF_ADDRESS_CREATED: "Dropoff address created successfully.",
    DROPOFF_ADDRESS_UPDATED: "Dropoff address updated successfully.",
    DROPOFF_ADDRESS_DELETED: "Dropoff address deleted successfully.",
    DASHBOARD_DATA_RETRIEVED: "Dashboard data retrieved successfully.",
    STATUS_CHANGED: "Status changed successfully",
    USER_ACCOUNT_DELETED: "User account deleted successfully.",
    DRIVER_ACCOUNT_DELETED: "Driver account deleted successfully.",
    REPORT_SUBMITTED: "Report submitted successfully.",
    REPORT_RETRIEVED: "Report retrieved successfully.",
    REPORT_NOT_FOUND: "Report Not Found.",
    TOKEN_UPDATED: "Token updated successfully.",
    LOCATION_UPDATED: "Location updated successfully.",
    NOTIFICATION_FETCHED: "Notifications fetched successfully.",
    NOTIFICATION_NOT_FOUND: "Notification not found.",
    PACKAGE_ACCEPTED: "Driver has accepted the package and will collect it soon.",
    PACKAGE_REJECTED: "Driver rejected the package.",
    PACKAGE_COMPLETED: "Driver delivered the package.",
    NO_DRIVER_AVAILABLE: "Drivers are busy or not available at the moment",
    PACKAGE_REASSIGNED: "The driver has rejected the package request. Searching for another driver...",
    NOTIFICATION_DELETED: "Notification deleted successfully"
};

export default messages